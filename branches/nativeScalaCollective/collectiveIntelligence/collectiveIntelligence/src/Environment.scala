	...
	loop
		{
			react
			{
			  ...
			  {
				  //println("UpdateSensor: " + sensorRange + "+-" + sensorDeltaRange + " +-"+  sensorDeltaAngle +"deg")
				   //println("UpdateSensor")
					 ...
					 */
					 if(world.contains(senderAgent))
					 {
						 ...
						 {
                            //println("detectedObstacle: " + obstacle + " agent: " + agent)
						    val vectorX: Double = obstacle.x - agent.x
						    val vectorY: Double = obstacle.y - agent.y
						    //println("vectorX: "+vectorX+ "vectorY: " + vectorY)
