...
class Environment( val minX: Int, val minY: Int, val maxX: Int, val maxY: Int,val scalaGui: Actor) extends Actor{
  ...
  //create border of obstacles around world limit
  for(y <- (minY-1) to (maxY+1)){
	  obstacles.add(minX-1,y,new Obstacle(obstacleType=2,minX-1,y))
	  obstacles.add(maxX+1,y,new Obstacle(obstacleType=2,maxX+1,y))
  }
  for(x <- (minX-1) to (maxX+1)){
	  obstacles.add(x,minY-1,new Obstacle(obstacleType=2,x,minY-1))
	  obstacles.add(x,maxY+1,new Obstacle(obstacleType=2,x,maxY+1))
  }
