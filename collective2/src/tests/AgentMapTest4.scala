...
object AgentMapTest4 extends SimpleGUIApplication{
    val width = 600
    val height = 400
    val environmentX = 11
    val environmentY = 10
    ...
    def top = new MainFrame{
        ...
        contents = new AgentMapTest4(environmentX,environmentY,width-20,height-50,
        		List( (4,1),(2,7) ), List( (2,4),(3,7),(4,2),(6,4),(7,7) ) )
...
class AgentMapTest4(val environmentX: Int, val environmentY: Int,
        ...
        case ButtonClicked(b) =>{
            if(b eq initializeButton){
                ...
                agentsWithLocations.foreach(agentWithLocation => {
                		...
                		if(first){
                			first = false
                			val ft =agent !! TestMapProducer( List( (2,2),(3,3),(3,5) ) )
                			ft()
                		}else{
